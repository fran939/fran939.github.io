...
// Kuudra gear and key items (hoverable inventory-style)
/*
async function buildKuudraGear(member, profile, me) {
  ...
  // G-Drag, gold collection, bank
  function getPets(m){
    const a = Array.isArray(m?.pets) ? m.pets : null;
    const b = Array.isArray(m?.pets_data?.pets) ? m.pets_data.pets : null;
    return a || b || [];
  const petsArr = getPets(member);
  ...
  // Also include items from all backpacks
  const bpMap2 = invRoot?.backpack_contents || member?.backpack_contents || null;
  let bpItems = [];
  if (bpMap2 && typeof bpMap2 === 'object') {
    const keys = Object.keys(bpMap2);
    for (const k of keys) {
  ...
  function findBest(setName) {
    ...
    return best;
  function hasReforge(it){
    try {
      const nameU = String(it?.name||'').toUpperCase();
      const reforges = ['ANCIENT','RENOWNED','NECROTIC','WISE','LOVING','GIANT','FIERCE','PURE','TITANIC','REINFORCED','SPIRITUAL','FABLED','WITHERED','PRECISE'];
      return reforges.some(r => nameU.includes(r+' '));
    } catch { return false; }
  function hasEnchants(it){
    try {
      const lore = Array.isArray(it?.lore)? it.lore : [];
      for (const raw of lore){
        const line = simplifyMcText(typeof raw === 'string' ? raw : JSON.stringify(raw));
        if (/\b[IVXLCDM]{1,4}\b/.test(line)) return true; // roman numerals
      }
    } catch {}
    return false;
  function hasEnchantAndReforge(it){ return hasReforge(it) && hasEnchants(it); }
  const terrorBest = findBest('terror');
  const auroraBest = findBest('aurora');
  const hollowBest = findBest('hollow');
  ...
  function matchItem(it, key) {
    const id = String(it?.hypixelId || it?.id || '').toUpperCase();
    const nm = String(it?.name || '').toUpperCase();
    return id.includes(key) || nm.includes(key.replace(/_/g,' '));
  const weaponKeys = ['HYPERION', 'TERMINATOR', 'BONEMERANG', 'RAGNAROK_AXE'];
  let weaponItems = allItems.filter(it => weaponKeys.some(k => matchItem(it, k)));
  // Include Katanas only when wearing Tuxedo
  const katanaItems = hasTux ? allItems.filter(it => matchItem(it, 'KATANA')) : [];
  weaponItems = weaponItems.concat(katanaItems);
  // Prefer Terminators with Duplex or Rend if any exist
  const termAll = weaponItems.filter(it => matchItem(it, 'TERMINATOR'));
  const hasEnchant = (it, names) => {
    const lore = it?.lore || [];
    const u = lore.map(l => simplifyMcText(typeof l === 'string' ? l : JSON.stringify(l)).toUpperCase());
    return names.some(n => u.some(line => line.includes(n)));
  };
  const termSpecial = termAll.filter(it => hasEnchant(it, ['DUPLEX', 'REND']));
  if (termSpecial.length) {
    weaponItems = weaponItems.filter(it => !matchItem(it, 'TERMINATOR')).concat(termSpecial);

  // Skill summary (from raw_profile.members[uuid].experience.SKILL_*)
  function levelFromSkillXp(xp) {
    ...
    // extend to 60 with 5m per level (approx)
    while (XP.length < 60) XP.push(5000000);
    const CUM = XP.reduce((a, v) => { a.push((a.at(-1)||0)+v); return a; }, []);
    let lvl = 0;
    for (let i=0;i<CUM.length;i++) { if (xp < CUM[i]) { const prev=i===0?0:CUM[i-1]; const seg=XP[i]; return i + (xp - prev)/seg; } }
    return XP.length;
  const expNode = member?.experience || member?.player_data?.experience || {};
  function getSkillExp(skill){ return Number(expNode?.[`SKILL_${skill}`] || 0); }
  const combatLvl = Math.floor(levelFromSkillXp(getSkillExp('COMBAT')));
  const foragingLvl = Math.floor(levelFromSkillXp(getSkillExp('FORAGING')));
  ...
  // Panels for armor & weapons (hoverable)
  const panels = [];
  // Armor helpers for slot and sorting
  const slotOf = (it) => {
  ...
  const tuxPiecesRaw = allItems.filter(it => /ELEGANT[_\s]?TUXEDO/i.test(String(it?.hypixelId||it?.name||'')));
  const tuxPieces = tuxPiecesRaw.filter(hasEnchantAndReforge);
  const hasTux = tuxPieces.some(it => /(CHESTPLATE|LEGGINGS|BOOTS)/i.test(String(it?.hypixelId||it?.name||'')));
  // (Tux panel appended later in requested order)
  // Terror column with helmet preference
  if (terrorBest) {
    const terrorAll = allItems.map(it => ({det: detectArmor(it), it})).filter(x => x.det && x.det.set==='terror');
    const bestTier = terrorBest.tier;
    const terrorItems = terrorAll.filter(x => x.det.tier === bestTier).map(x => x.it);
    const terrorHelmet = terrorItems.find(it => /HELMET/i.test(String(it?.hypixelId||it?.name||'')));
    let helm = null;
    if (hasTux) { helm = primordialHelm || terrorHelmet || warden || null; }
    else { helm = warden || primordialHelm || terrorHelmet || null; }
    const chest = terrorItems.find(it => /CHESTPLATE/i.test(String(it?.hypixelId||it?.name||'')));
    const legs = terrorItems.find(it => /LEGGINGS/i.test(String(it?.hypixelId||it?.name||'')));
    const boots = terrorItems.find(it => /BOOTS/i.test(String(it?.hypixelId||it?.name||'')));
  ...
  // Mage armor ordering follows
  // Terror (Mana) — LOVING/NECROTIC only; helmet: Aurora Helm, then Wither Goggles, then Terror helm
  if (terrorBest) {
    const bestTier = terrorBest.tier;
    const terrorManaAll = allItems
      .map(it => ({ it, nameU: String(it?.name||'').toUpperCase(), det: detectArmor(it) }))
      .filter(x => x.det && x.det.set==='terror' && x.det.tier===bestTier && /\b(NECROTIC|LOVING)\b/.test(x.nameU))
      .map(x => x.it);
    if (terrorManaAll.length) {
      const tHelm = terrorManaAll.find(it => /HELMET/i.test(String(it?.hypixelId||it?.name||'')));
      const chest = terrorManaAll.find(it => /CHESTPLATE/i.test(String(it?.hypixelId||it?.name||'')));
      const legs = terrorManaAll.find(it => /LEGGINGS/i.test(String(it?.hypixelId||it?.name||'')));
      const boots = terrorManaAll.find(it => /BOOTS/i.test(String(it?.hypixelId||it?.name||'')));
      const prefHelm = (allItems.find(it => /AURORA/i.test(String(it?.hypixelId||it?.name||'')) && /HELMET/i.test(String(it?.hypixelId||it?.name||'')))
        || allItems.find(it => /WITHER[_\s]?GOGGLES/i.test(String(it?.hypixelId||it?.name||'')))
        || tHelm || null);
  ...
  // Aurora vertical (only show pieces that have both enchants and a reforge)
  if (auroraBest) {
    const bestTier = auroraBest.tier;
    const aur = allItems
      .map(it => ({det: detectArmor(it), it}))
  ...
  // Aurora (filtered) is added above; append Tuxedo after mage armor
  if (hasTux) {
  ...
  if (weaponItems.length) {
    panels.push(`<div><h3>Weapons</h3>${renderItemGrid2(weaponItems, { showSlot: false })}</div>`);

  // Arrange armor/weapons columns side-by-side to reduce scrolling
  const panelsHtml = panels.length
    ? `<div class="panel-inner" style="display:flex;flex-wrap:wrap;gap:16px;align-items:flex-start">${panels.map(p => `<div style="display:flex;flex-direction:column;gap:8px">${p}</div>`).join('')}</div>`
    : '';
  // Add skills row (compact)
